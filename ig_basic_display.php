<?php require_once('instagram_defines.php'); ?>
<?php
...
class instagram_basic_display_api
{
    ...
    private $_redirectUrl = IG_REDIRECT_URL;
    private $_getCode = '';
    ...
    private function _getUserAccessToken()
    {
        ...
        $response = $this->_makeApiCall($params, '_getUserAccessToken');
        return $response;
    }
    ...
    private function _getLongLivedUserAccessToken()
    {
        ...
        $response = $this->_makeApiCall($params, '_getLongLivedUserAccessToken');
        return $response;
    }
    ...
    public function getUser()
    {
        ...
        $response = $this->_makeApiCall($params, 'getUser');
        return $response;
    }
    ...
    public function getUsersMedia()
    {
        ...
        $response = $this->_makeApiCall($params, 'getUsersMedia');
        return $response;
    }
    ...
    public function getPaging($pagingEndpoint)
    {
        ...
        $response = $this->_makeApiCall($params, 'getPaging');
        return $response;
    }
    ...
    public function getMedia($mediaId)
    {
        ...
        $response = $this->_makeApiCall($params, 'getMedia');
        return $response;
    }
    ...
    public function getMediaChildren($mediaId)
    {
        ...
        $response = $this->_makeApiCall($params, 'getMediaChildren');
        return $response;
    }

    private function _makeApiCall($params, $fnc)
    {
        $ch = curl_init();

        $endpoint = $params['endpoint_url'];
        if ('POST' == $params['type']) { // Post isteği
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($params['url_params']));
            curl_setopt($ch, CURLOPT_POST, 1);
        } elseif ('GET' == $params['type'] && false === isset($params['url_params']['paging'])) { // Get isteği
            $params['url_params']['access_token'] = $this->_userAccessToken;
            // Parametreleri endpoint'e ekleyin
            $endpoint .= '?' . http_build_query($params['url_params']);
        }

        // Genel CURL seçenekleri
        curl_setopt($ch, CURLOPT_URL, $endpoint);
        curl_setopt($ch, CURLOPT_TIMEOUT, 5); // Örnek olarak, 5 saniye timeout
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($ch);

        // CURL hatasını kontrol edin
        // if (curl_errno($ch)) {
        //     throw new Exception('CURL Hatası: ' . curl_error($ch));
        // }

        // HTTP hata kodunu kontrol edin
